...
# Domain-specific utilities for sensor nomenclature and sorting
...
class RealtimePlotWidget(QWidget):
    def __init__(self, time_window_s: float = 60.0, parent=None):
        ...
        # Internal state maps for data buffering and curve referencing
        ...
        # Restrict user manipulation to maintain automated scaling integrity
        self._plot_widget.setMouseEnabled(x=False, y=False)
        self._plot_widget.hideButtons()
        
        self._plot_widget.setXRange(0, self._time_window, padding=0.02)

        # Initialize the secondary Y-axis overlay for voltage telemetry
        ...
        # Configure the interactive legend
    ...
    def set_reference_mode(self, enabled: bool):
        """Toggles dedicated styling parameters for the reference thermal probe."""
    ...
    def _update_views(self):
        # Synchronize the bounding geometries of the overlapping ViewBoxes
    ...
    def clear(self):
        """Purges all plotted data, geometry references, and resets axes."""
        ...
        # Safely decouple the existing legend to prevent phantom rendering artifacts
    ...
    def add_point(self, t_s: float, values: Dict[str, float]):
        ...
        # Batch update graphical items for optimal rendering performance
        ...
        # Dynamically scale the X-axis domain to accommodate incoming temporal data
        view_max = max(self._time_window, current_max_time)
        self._plot_widget.setXRange(0, view_max, padding=0.02)

        # Compute required bounds for dynamic Y-axis auto-scaling
        temp_vals = []
        volt_vals = []
    ...
    def _create_curve(self, key: str):
        ...
        # Assign thematic identifiers based on sensor classification
        if is_reference:
        ...
        # Define line styles separating reference, target, and empirical data
        if is_reference:
            style = Qt.DashLine    
            width = 2
            symbol = None
        ...
        # Mount the curve object to the appropriate axis coordinate space
        if use_right_axis:
