...
class PartThreeMeasurement(StreamingTempMeasurement):
    ...
    def __init__(self, serial_mgr, target_temp=25.0, on_steady_state=None):
        ...
        # Instantiate localized PID controller with asymmetric tuning parameters
        ...
        # State tracking parameters for automated steady-state detection
        self.on_steady_state = on_steady_state
        ...
        # Criteria required to qualify the system as thermally stable
    ...
    def on_stop(self):
        super().on_stop()
        # Guarantee hardware shutdown of thermal elements upon completion
    ...
    def perform_regulation_logic(self, values: dict) -> dict:
        ...
        if current_temp is not None:
            # Compute requisite control action based on thermal delta
            ...
            # Transmit updated control signals if the operational state changed
            if self.serial.is_open():
            ...
            # Retroactively inject controller state into the export buffer
            if self.recorded_data:
                self.recorded_data[-1]["PWM"] = values["PWM"]
                self.recorded_data[-1]["Target"] = values["Target"]

            # Manage automated data acquisition phase transitions
            if self.meas_state != "DOMĚŘENO":
                ...
                if self.meas_state == "ČEKÁNÍ":
                    ...
                    # Buffer only data points collected during the steady-state period
                    if self.recorded_data:
    ...
    def export_to_csv(self, filename: str, allowed_sensors=None) -> bool:
        # Enforce inclusion of BME humidity mapping during export
        if allowed_sensors is not None:
            allowed_sensors = set(allowed_sensors)
            allowed_sensors.update(["H_BME"])
            
        # Temporarily substitute the primary datastore with the steady-state subset
