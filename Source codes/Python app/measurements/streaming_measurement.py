...
class StreamingTempMeasurement(BaseMeasurement):
    ...
    def on_start(self):
        ...
        # Spin up a background watcher to manage duration limits asynchronously
    ...
    def handle_line(self, line: str):
        ...
        # Synchronize local timeline with hardware micro-controller ticks
        t_ms = msg.get("t_ms")
    ...
    def _watchdog_loop(self):
        while not self._stop_flag and self.is_running():
            ...
            # Subsystem timeout check
            if (now - self._last_data_time) > self.NO_DATA_TIMEOUT_S:
                pass
            
            # Enforce hard measurement duration limit
